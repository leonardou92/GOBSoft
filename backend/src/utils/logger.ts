...
export function logError(context: string, error: unknown, extra?: LogExtra) {
  ...
  // También dejamos el error en consola para desarrollo, pero con formato más legible.
  ...
  if (error instanceof Error && error.stack) {
    // Evitar duplicar la primera línea del stack
    // (normalmente "ErrorName: message"), ya que ya se imprimió arriba.
    const stackLines = error.stack.split('\n');
    const trimmedStack =
      stackLines.length > 1 ? stackLines.slice(1).join('\n') : error.stack;
